...
// Middlewares de logging (apenas para desenvolvimento)
if (process.env.NODE_ENV !== 'production') {
    app.use((req, res, next) => {
        ...
        res.on('finish', () => {
            console.log({
                statusCode: res.statusCode, // Código de status HTTP
            });
        });
        next();
    });
...
app.use((req, res, next) => {
    ...
    if (!logData.userId || logData.url === "/favicon.ico" || logData.url === "/") {
        return next();
    }
    ...
    res.on('finish', async () => {
        try {
            if (logData.url === "/logout") {
                accessGranted = res.statusCode = 200;
            }
            accessGranted = res.statusCode === 200;
            ...
            console.error('Erro ao registrar o log de acesso:', error);
        }
    });
    next();
});
